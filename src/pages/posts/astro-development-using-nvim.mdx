...
Call this command at some point during neovim initialization. I have a file named `ft-add.lua` which I require in my root `init.lua` file.

We only need to match the `.mdx` extension, but check out the linked documentation to learn about other options available for filetype detection.
