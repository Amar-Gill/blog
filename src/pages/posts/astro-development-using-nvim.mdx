...
- Use an autocommand to detect the `.mdx` filetype. Otherwise use the [jxnblk/vim-mdx-js](https://github.com/jxnblk/vim-mdx-js) plugin for syntax highlighting of `.mdx` files. (neovim 0.8.2)
...
I couldn't find a tree-sitter grammar for `.mdx` files, but I have the tree-sitter grammar for [markdown](https://github.com/MDeiml/tree-sitter-markdown) installed. So I created an autocommand that set the file type to `markdown.mdx` when I enter a new buffer:
