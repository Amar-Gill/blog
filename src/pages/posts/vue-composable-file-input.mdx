...
description: Using the Vue 3 composition api to obtain the base 64 encoded url for a file plus it's file size.
...
I want to share a simple composable I used in a Vue project of mine. I needed to upload a file to the backend, so I needed the base 64 encoded url.

Encoding a file to the base 64 format allows you to send the file to a server through http, since the encoded url is just a string.
...
- We make use of the JavaScript [FileReader](https://developer.mozilla.org/en-US/docs/Web/API/FileReader) api to obtain the base 64 encoded url
