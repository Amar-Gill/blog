...
It would be nice to have your generated migrations be applied automatically when you push code up for a deployment, instead of copying the SQL scripts and applying them manually.
...
In the second method, we will leverage the [nuxt-prepare module](https://nuxt.com/modules/prepare) instead of our own lifecycle hook handler. Start by adding the module to your `nuxt.config.ts` file and create a new `server.prepare.ts` file in your project root.
