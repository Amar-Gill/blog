...
title: Build An Address Lookup Feature In A React Application
publishDate: 13 January 2023
description: Learn how to use the open source LeafletJS package in React, to create a map with address search capabilities.
...
[Leaflet](https://leafletjs.com/) is an open source JavaScript package that lets you easily add an interactive map into your app or webpage. It has a small footprint yet is fully featured, and has a thriving community of [plugins](https://leafletjs.com/plugins.html) to choose from.
...
At time of writing the package versions used are:
...
Let's make something useful now. We will leverage the [Geoapify Address Autocomplete API](https://www.geoapify.com/address-autocomplete) to add address search functionality to our map. It generously provides 3000 daily credits for use in the free tier. We will use the [Geocoder Autocomplete](https://github.com/geoapify/geocoder-autocomplete) package which handles making API calls, and rendering of the search suggestions on the page for us.
...
<aside className="caution mb-5">
  Depending on the popularity of this post, there might not be credits remaining on the Geoapify free tier.
