...
[Leaflet](https://leafletjs.com/) is an open source JavaScript package that let's you easily add an interactive map into your app or webpage. It has a thriving community of [plugins](https://leafletjs.com/plugins.html) to choose from.

The [react-leaflet](https://react-leaflet.js.org/) package provides React bindings for the Leaflet package. In this post I will demonstrate how to create a custom address search component in a React app using the [react-leaflet](https://react-leaflet.js.org/) package and [Geoapify](https://www.geoapify.com/) API.
...
Package versions used at time of writing:
- react
- react-dom
- leaflet
- react-leaflet
...
Add it as a child component of `<MapContainer>`. We will see a pink square in the top right corner of the map, with a click event handler.
...
- A Leaflet control is an instance of the `Control` class with the additional `onAdd` and `onRemove` methods.
- The `useMap` hook returns the root `<MapContainer>` reference for any of its child components.
- The `Control.extend` method returns a class that implements the leaflet `Control` type, and we define the position as `topright`.
...
Let's make something useful now. We will leverage the [Geoapify Address Autocomplete API](https://www.geoapify.com/address-autocomplete) to add address search functionality to our map. It has a generous request limit of 3000 daily credits for the free tier. We will use the [Geocoder Autocomplete](https://github.com/geoapify/geocoder-autocomplete) package which handles making API calls, and rendering of the search suggestions on the page for us.

<aside className="info">
There is a [react-geocoder-autocomplete](https://github.com/geoapify/react-geocoder-autocomplete) package available as well. We will *not* be using this package. Because the Leaflet custom control API works by returning an `HTMLElement` from the `onAdd` method, as shown in the example above using an `HTMLDivElement`.
</aside>
...
<aside className="caution mb-5">
Depending on how popular this post is, the Geoapify API free tier might run out of credits. Please use sparingly =]
</aside>
<LeafletMap showAddressSearch={true} />
...
Like in the previous example, we add styles to the `HTMLElement`. The `minimal` CSS class is from the stylesheet provided by the Geocoder Autocomplete package.

<aside className="info">
The `HTMLElement` container must have a CSS display property of `relative` or `absolute`.
</aside>

The Autocomplete component can [listen to a number of events](https://github.com/geoapify/geocoder-autocomplete#geocoder-autocomplete-events). We use the `select` event to change the map view when a suggestion is selected.

The `click` and `dblclick` event handlers are used to stop the event bubbling up to the root `<MapContainer>` element. You might want this behaviour if you have `click` event handlers on the `<MapContainer>` component itself. For example, you can add a `<Marker>` and set the view to the location that was clicked on the map tile.
